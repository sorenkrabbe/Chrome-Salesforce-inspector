...
class ApiExploreVm {
  ...
  editSubUrl(subUrl) {
    let url = prompt("REST API url", subUrl.apiUrl);
    if (url) {
      let args = new URLSearchParams();
      args.set("host", sfHost);
      args.set("apiUrl", url);
      location.href = "explore-api.html?" + args;
    }
  }
  ...
  performRequest(apiPromise) {
    this.spinFor(Promise.all([apiPromise, askSalesforceSoap("/services/Soap/u/" + apiVersion, "urn:partner.soap.sforce.com", "<getUserInfo/>")]).then(([result, userInfo]) => {
